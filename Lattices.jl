...
export Lattice, MakeLattice, PlotNeighbors
    ...
    function MakeLattice(name::ASCIIString,l::UInt8)
        if(name=="Square")
            return MakeSquare(l)
        elseif(name=="Chain")
            return MakeChain(l)
    ...
    function MakeHoneycomb(l::UInt8)
        d::UInt8=2;
        nnei::UInt8=3;
        N::UInt16=2*l^2;
        a=[[2*cos(pi/6) 0]
    ...
    function MakeX(a::Array,unit::Array,l::UInt8,d::UInt8)
        ncell=size(a)[1];
        N=ncell*l^d;
